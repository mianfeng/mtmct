...
cv::Mat VisualizeResult(
    ...
    const std::vector<std::string> &lables, const std::vector<int> &colormap,
    ...
    for (int i = 0; i < results.size(); ++i) {
        ...
        if (is_rbox) {
            ...
            cv::Rect roi =
                cv::Rect(results[i].rect[0], results[i].rect[1], w, h);
            ...
            if (mask_v.size() > 0) {
                ...
                std::memcpy(mask.data, mask_v.data(),
                            mask_v.size() * sizeof(int));
                ...
                cv::drawContours(colored_img, contours, -1, roi_color, -1,
    ...
    for (int im_id = 0; im_id < mats.size(); im_id++) {
        ...
        for (int j = start_idx; j < start_idx + bbox_num[im_id]; j++) {
            if (is_rbox) {
                // Class id
                int class_id =
                    static_cast<int>(round(output_data_[0 + j * 10]));
                // Confidence score
                ...
                if (config_.mask_) {
                    ...
                    for (int k = 0; k < out_mask_dim; ++k) {
                        if (output_mask_data_[k + j * out_mask_dim] > -1) {
                            mask.push_back(
                                output_mask_data_[k + j * out_mask_dim]);
                        }
                    }
            ...
            if (out_score_data_[bbox_id] >= threshold) {
                ...
                for (int k = 0; k < mat.rows * mat.cols; ++k) {
                    global_mask.push_back(static_cast<int>(
                        out_global_mask_data_[k +
                                              bbox_id * mat.rows * mat.cols]));
                }
...
void ObjectDetector::Predict(const std::vector<cv::Mat> imgs,
    ...
    if (batch_size > 1 && CheckDynamicInput(in_net_img_all)) {
        ...
        for (int bs_idx = 0; bs_idx < batch_size; bs_idx++) {
            ...
            in_data_all.insert(in_data_all.end(), pad_data.begin(),
                               pad_data.end());
        }
        // update in_net_shape
        inputs_.in_net_shape_ = {static_cast<float>(rh),
    ...
    if (config_.arch_ == "SOLOv2") {
        ...
        for (int i = 0; i < warmup; i++) {
            ...
            for (int j = 0; j < output_names.size(); j++) {
                auto output_tensor =
                    predictor_->GetOutputHandle(output_names[j]);
        ...
        for (int i = 0; i < warmup; i++) {
            ...
            for (int j = 0; j < output_names.size(); j++) {
                ...
                } else if (output_tensor->type() ==
                           paddle_infer::DataType::INT32) {
                    out_bbox_num_data_.resize(out_num);
                    output_tensor->CopyToCpu(out_bbox_num_data_.data());
                } else {
    ...
    if (config_.arch_ == "PicoDet") {
        ...
        SOLOv2Postprocess(imgs, result, bbox_num, out_bbox_num_data_,
                          out_label_data_, out_score_data_,
                          out_global_mask_data_, threshold);
    } else {
        is_rbox =
            output_shape_list[0][output_shape_list[0].size() - 1] % 10 == 0;
    ...
    std::chrono::duration<float, std::milli> postprocess_diff_ms =
        std::chrono::duration_cast<std::chrono::duration<float, std::milli>>(
            postprocess_diff);
    double promilliseconds = postprocess_diff_ms.count();
    // SPDLOG_INFO("yolo Preprocess: {} ms, Inference: {} ms, Postprocess: {}
    // ms",
    //             premilliseconds, milliseconds, promilliseconds);
}
