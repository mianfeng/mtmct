...
void PredictImage(const std::vector<std::string> all_img_paths,
                  const int batch_size, const double threshold,
    ...
    for (int idx = 0; idx < steps; idx++) {
        ...
        for (int bs = 0; bs < left_image_cnt; bs++) {
            std::string image_file_path =
                all_img_paths.at(idx * batch_size + bs);
        ...
        bool is_rbox = false;
        if (run_benchmark) {
            det->Predict(batch_imgs, threshold, 10, 10, &result, &bbox_num,
                         &det_times);
        } else {
            det->Predict(batch_imgs, threshold, 0, 1, &result, &bbox_num,
                         &det_times);
            ...
            for (int i = 0; i < left_image_cnt; i++) {
                ...
                for (int j = 0; j < bbox_num[i]; j++) {
                    ...
                    if (item.rect.size() > 6) {
                        is_rbox = true;
                        printf(
                            "class=%d confidence=%.4f rect=[%d %d %d %d %d %d "
                            "%d %d]\n",
                            item.class_id, item.confidence, item.rect[0],
                            item.rect[1], item.rect[2], item.rect[3],
                            item.rect[4], item.rect[5], item.rect[6],
                            item.rect[7]);
                    } else {
                        printf("class=%d confidence=%.4f rect=[%d %d %d %d]\n",
                               item.class_id, item.confidence, item.rect[0],
                ...
                std::string image_file_path =
                    all_img_paths.at(idx * batch_size + i);
                output_path += image_file_path.substr(
                    image_file_path.find_last_of('/') + 1);
