...
#include "include/reid_postprocess.h"
...
// Visualiztion reid Result
void VisualizeReidResult(const std::vector<Match>& match,
                         const CameraResult& results1,
                         const CameraResult& results2);
class ReidDetector {
   public:
    explicit ReidDetector(const std::string& model_dir,
                          ...
                          bool trt_calib_mode = false, bool use_dark = true) {
        this->device_ = device;
        this->gpu_id_ = gpu_id;
    ...
    // Run predictor
    void Predict(const std::vector<cv::Mat> imgs, std::vector<int> rect,
                 const double threshold = 0.6, const int warmup = 0,
                 const int repeats = 1, CameraResult* results = nullptr,
                 int num = 0, std::vector<double>* times = nullptr);
    void Predict(const cv::Mat& img, const double threshold = 0.6,
                 PaddleDetection::CameraResult* results = nullptr);
   ...
   private:
    ...
    void Preprocess(const cv::Mat& oriImg, ImageBlob* data,
                    const std::vector<double>& mean = {0.485, 0.456, 0.406},
                    const std::vector<double>& std = {0.229, 0.224, 0.225});
    // Postprocess result
    void Postprocess(std::vector<float>& output, std::vector<int> output_shape,
                     std::vector<int64_t>& idxout, std::vector<int> idx_shape,
                     CameraResult* results,
